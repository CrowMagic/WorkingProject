...
{
    if ([tableView isEqual:_cView.couponTable]) {
        return 0;
    }
    else
    {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    else
    {
        ...
        if (headModel.headState == 1) {
            ...
            UILabel *subLabel = [[UILabel alloc] initWithFrame:CGRectMake([Util ReturnViewFrame:label Direction:@"X"], 10, SCREEN_WIDTH - ([Util ReturnViewFrame:label Direction:@"X"] + 10) , 20)];
            subLabel.textColor = [UIColor grayColor];
            subLabel.textAlignment = NSTextAlignmentLeft;
            subLabel.font = [UIFont systemFontOfSize:12.0];
            [view addSubview:subLabel];
            _editBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            _editBtn.frame = CGRectMake(290, 0, 80, 40);
            _editBtn.tag = section;
            [_editBtn setTitle:@"领取|编辑" forState:UIControlStateNormal];
...
{
   // [self.delegate creatViewWithButtonTag:sender.tag];
    
    _cView = [[CouponView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    [self addSubview:_cView];
    UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(dissMissView)];
    [_cView.alphaView addGestureRecognizer:tapGestureRecognizer];
    
    _cView.couponTable.delegate = self;
    _cView.couponTable.dataSource = self;
    [_cView.couponTable registerNib:[UINib nibWithNibName:@"CouponTableViewCell" bundle:nil] forCellReuseIdentifier:@"conCell"];
}
-(void)dissMissView
{
    [UIView animateWithDuration:0.5 animations:^{
        _cView.frame = CGRectMake(0, hKscreen*60, 0, 0);
    }];
}
#pragma mark 底部间距
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tableView isEqual:_cView.couponTable]) {
        CouponTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"conCell"];
        cell.nameLbl.text = @"南玉南玉南玉南玉南玉";
        cell.timeLbl.text = @"2016-6-1至2016-9-1";
        ...
        cell.storeImageView.image = [UIImage imageNamed:@"1"];
        cell.orderLbl.text = @"玉玉玉玉玉玉玉玉玉玉玉玉";
        return cell;
    }
    else
    {
