...
- (void)selectButtonAction:(UIButton *)bu
{
    ...
    if (bu.selected) {
        ...
        //[self addSubview:self.delView];//添加底部视图
        [UIView animateWithDuration:0.1f animations:^{
            _delView.frame = CGRectMake(0, hKscreen-64-44, wKscreen, 44);
            self.selectAllButton.frame = CGRectMake(10, 0, 100, 40);
            self.deleteButton.frame = CGRectMake(200, 0, 100, 40);
        }];
    }
    else {
        self.isCanEdit = NO;
        ...
        [UIView animateWithDuration:0.1f animations:^{
            _delView.frame = CGRectMake(0, hKscreen-64-44, 320, 0);
            self.selectAllButton.frame = CGRectMake(0, 0, 0, 0);
