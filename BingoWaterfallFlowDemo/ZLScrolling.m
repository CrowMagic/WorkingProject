...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    _titleLbl = [[UILabel alloc]initWithFrame:CGRectMake(0, 40, self.frame.size.width, 30)];
    
    _titleLbl.text = [NSString stringWithFormat:@"my is cell title %d",indexPath.row];
    // 判断图片来源
    if ([self.photos[0] isKindOfClass:[NSURL class]]) {
    ...
    [cell.contentView addSubview:_titleLbl];
    return cell;
}
...
- (UIPageControl *)pageControl {
    if (!_pageControl) {
        _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(20, self.frame.size.height-20, self.frame.size.width-20, 20)];
